...
$("document").ready(function() {
  ...
  sendRequest(stateObject.bookName, stateObject.pageNo, true);
  };
});
...
function sendRequest(bookName, pageNo, callFromPopState) {
  ...
  callApi(currentBookName, currentPage, callFromPopState);
...
function callApi(bookName, pageNo, callFromPopState) {
  var url = "http://it-ebooks-api.info/v1/search/";
  url += bookName + "/page/" + pageNo;
  var stateObj = { bookName: bookName, pageNo: pageNo };
  //historyUrl = bookName + "/" + pageNo;
  if(!callFromPopState) {
    history.pushState(stateObj, pageNo,"/" + bookName + "/" + pageNo);
    console.log(history.state);
    console.log(historyObj);
  }
